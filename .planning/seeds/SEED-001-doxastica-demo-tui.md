...
status: dormant
planted: 2026-06-14
planted_during: Phase 1 (M0 — Protocol, Backend Port & Data-Model Decisions)
trigger_when: M0 exit gate green (post-v1) — after the AGM/Hansson conformance suite passes
scope: Large
---

# SEED-001: doxastica-demo — TUI "belief inspector" showcasing belief-vs-retrieval

> **Status:** parked idea, not part of M0 scope. Build *after* the v1 library (M0 exit
> gate) is green. Recorded so it isn't lost and can lightly inform M0 decisions (esp. the
> in-memory backend and API ergonomics). Flesh out scenario scripts later with
> `/gsd-explore` or mock the UI with `/gsd-sketch`.

## Why This Matters

Every agent-memory product today is a **retrieval** system; doxastica is a **belief**
system — it knows what an agent holds true, what's *actually* true, what the agent got
*wrong*, *why*, and what *changes* when a fact is retracted. None of those are retrieval
questions; a RAG agent cannot answer them. The demo is the artifact that makes that reframe
legible to an "AI agents for business" audience.

## When to Surface

**Trigger:** M0 exit gate green (post-v1). Build *after* the standalone library ships its
green conformance suite. Surfaces during `/gsd-new-milestone` once M0 is complete and a
post-v1 / demo / go-to-market milestone is being scoped.

## Scope Estimate

**Large** — a standalone companion app (`doxastica-demo`), its own milestone-sized effort.
Adds **no** M0 work and must not add demo requirements to REQUIREMENTS.md.
...
doxastica's *novel, non-retrieval* capabilities. The LLM (if any) is just glue that turns
scenario events into belief operations (`revise`/`expand`/`add_edge`). The novel capability
on stage is doxastica's.
...
- Runs on the **in-memory backend** — so the demo *also* exercises the pluggable-backend
  extension, and needs no LadybugDB file or external service. (Three-for-one: the demo that
  shows extensions #1 multi-scope and #2 time-travel *runs on* extension #3 backends.)
...
- A clean way to **replay a scripted event stream** into belief ops, and to ask for "state
  as of event N," makes both the demo and the test harness nicer — `get_scope_at` already
  covers the latter.
- Nothing here changes the M0 exit gate; do not add demo requirements to REQUIREMENTS.md.

## Breadcrumbs

- `.planning/PROJECT.md` — extensions: multi-scope, time-travel (`get_scope_at`), pluggable backends
- `.planning/REQUIREMENTS.md` — BACK-03 (in-memory backend), FORMAL-06, HIST-03 (`get_scope_at`), EDGE-02 (`get_impact`)
- `.planning/ROADMAP.md` — Phase 7 (irony join + conformance oracle), Phase 5 (`get_impact` cascade)
- Migrated verbatim from the former `.planning/DEMO.md` (commit `03727d7`).
