...
The **executable form** of this contract is the parameterised conformance suite, which the
LadybugDB reference adapter and the in-memory oracle both pass identically.
...
- **`traverse(start, edge_types, max_depth, direction="out") -> (reached, frontier)`** — the
  **single** graph-walk primitive. Following only edges in `edge_types` from `start`, it returns:
  ...
  [Ordering left to the core](#5-ordering-left-to-the-core)).
